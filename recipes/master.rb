...
runit_service 'kube_controller_manager' do
  ...
  template_name 'docker'
  options(
    run_args: '--name=kubernetes-controller-manager -i --rm --net=host -v /var/run/docker.sock:/var/run/docker.sock',
    image: "gcr.io/google_containers/hyperkube:#{node['kubernetes']['version']}",
...
runit_service 'kube_scheduler' do
  ...
  template_name 'docker'
  options(
    run_args: '--name=kubernetes-sheduler -i --rm --net=host -v /var/run/docker.sock:/var/run/docker.sock',
    image: "gcr.io/google_containers/hyperkube:#{node['kubernetes']['version']}",
...
runit_service 'kubelet' do
  ...
  options(
    ...
    image: "gcr.io/google_containers/hyperkube:#{node['kubernetes']['version']}",
    command: "/hyperkube kubelet --api_servers=http://#{internal_ip}:8080 --v=2 --address=#{internal_ip} --enable_server --hostname_override=#{internal_ip} --allow-privileged --cluster_dns=#{node['kubernetes']['cluster_dns']} --cluster_domain=#{node['kubernetes']['cluster_domain']}"
  )
end
